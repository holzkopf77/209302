...
/*!
 * Funkcja sortuje tablice typu int za pomoca algorytmu quicksort
 * z wyborem pivota na podstawie mediany 3 elementow
 * \param[] tablica - typ int*, wskaznik na tablice do posortowania
 * \param[] lewy - typ int, lewy indeks tablicy
 * \param[] prawy - typ int, prawy indeks tablicy
 */
void quicksortMediana(int *tablica, int lewy, int prawy)
{
  ...
  int v;
  if(tablica[lewy]<srodek)
    {
      if(tablica[prawy]<tablica[lewy])v=tablica[prawy];
      else v=tablica[lewy];
    }
  else
    {
      if(tablica[prawy]<srodek)v=tablica[prawy];
      else v=srodek;
    }
    int i,j,x;
    i=lewy;
...
/*!
 * Funkcja obudowuje funkcje quicksort, zeby mogla zostac wczytana przez klase Benchmark
 * \param[] tablica - typ int*, wskaznik na tablice do posortowania
...
void ObudowaQuickSortMediana(int *tablica,int rozmiar)
{
  int lewy=0;
  int prawy=rozmiar-1;
  quicksortMediana(tablica,lewy,prawy);
}
...
void ObudowaMergeSortowanie(Stack<int> *tab,int rozmiar)
{
  int lewy=0;
  int prawy=rozmiar-1;
  MergeSortowanie(tab,lewy, prawy);
}
